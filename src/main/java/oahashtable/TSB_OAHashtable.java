...
public class TSB_OAHashtable<K,V> implements Map<K,V>, Cloneable, Serializable
{
    ...
    public V get(Object key)
    {
        ...
        if(x != null)
        {
            if(x.getKey().getClass() != key.getClass()) {
                throw new ClassCastException("get(): la clase de key no es compatible con la tabla.");
            }
            valor = x.getValue();
        }
        return valor;
    ...
    public V remove(Object key)
    {
        ...
        int ik = this.h((K)key);
        Entry<K,V> entry =(Entry<K, V>) this.search_for_entry((K)key, ik);
        V old = null;
        if(entry != null)
        {
            old = entry.getValue();
    ...
    private class KeySet extends AbstractSet<K>
    {
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            public boolean hasNext()
            {
                ...
                if(current >= t.length) { return false; }
            ...
            public K next()
            {
                ...
                int next = current;
                Entry<K, V> t[] = (Entry<K, V>[]) TSB_OAHashtable.this.table;
                for (next++ ; t[next].state != TSB_OAHashtable.CLOSED; next++);
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>>
    {
        ...
        public boolean contains(Object o)
        {
            ...
            int ik = TSB_OAHashtable.this.h(entry.getKey());
            Entry<K, V> entryTable = (Entry<K, V>) TSB_OAHashtable.this.search_for_entry(entry.getKey(), ik);
            if(entryTable != null && entryTable.state == TSB_OAHashtable.CLOSED) return true;
            return false;
        }
        ...
        public boolean remove(Object o)
        {
            ...
            int ik = TSB_OAHashtable.this.h(entry.getKey());
            int index = TSB_OAHashtable.this.search_for_index(entry.getKey(), ik);
            if(index != -1){
                ((Entry<K,V>) TSB_OAHashtable.this.table[index]).setState(TSB_OAHashtable.TOMBSTONE);
                TSB_OAHashtable.this.count--;
                TSB_OAHashtable.this.modCount++;
                return true;
            }
